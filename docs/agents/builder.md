...
- `_step_node.html.erb` — one `role="treeitem"`: its row, its exit doors in a `role="group"` (a foldable `<details>` per exit that holds steps), then its continuation door. An exit's door is a wired chip (a fold's `<summary>`), a stub (ONE button, the grow trigger) or a jump (ONE button naming the target, with its type icon: "Yes → Working · step 4"); a wired continuation is a quiet "↓ label" connector, and a plain wired Next draws nothing. No type colour anywhere in the list (UIGUIDE § Step-type hues); extras render as dashed chips under the exits
