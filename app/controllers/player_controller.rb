class PlayerController < ApplicationController
  ...
  def start
    ...
    redirect_to player_scenario_step_path(Scenario.start!(workflow, user: current_user, purpose: "live"))
  end
  ...
  def show_shared
    ...
    landed = Scenario.start!(@workflow, user: current_user, purpose: "live", shared_access: true)
  ...
  def runner_step_path(scenario) = player_scenario_step_path(scenario)
  def runner_results_path(scenario) = player_scenario_show_path(scenario)
  def runner_next_path(scenario) = player_scenario_next_path(scenario)
  def runner_stop_path(scenario) = player_scenario_stop_path(scenario)
  def runner_back_path(scenario) = player_scenario_back_path(scenario)

  # Hidden when nobody is signed in: `stop` is not in the authenticate_user!
  # skip list, so an anonymous visitor clicking Cancel would be bounced to a
  # sign-in page. The gate is the session, not shared_access.
  def runner_shows_cancel? = current_user.present?
