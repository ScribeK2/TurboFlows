...
module WorkflowParsers
  class MarkdownParser < BaseParser
    ...
    def extract_description(lines)
      ...
      lines.each_with_index do |line, index|
        ...
        if found_title && !stripped.match(/^##\s+Step|^##\s+Steps|^###\s+Step|^\d+\./)
          if /^##\s+(.+)$/.match?(stripped) || (in_description && stripped.empty?)
            break
    ...
    def parse_step_line(current_step, stripped)
      ...
      append_description(current_step, stripped)
    end
    ...
    def append_description(current_step, stripped)
      ...
      heading_pattern = /^##|^###|^\d+\./
      is_recognized = stripped.match(field_pattern) || stripped.match(heading_pattern) || stripped.empty?
      if !is_recognized && current_step[:description].exclude?(stripped)
    ...
    def parse_markdown_options(options_str)
      options_str.split(',').filter_map do |opt|
        opt = opt.strip
    ...
    def parse_markdown_transitions(transitions_str)
      transitions_str.split(',').filter_map do |t|
        t = t.strip
        next nil if t.blank?
