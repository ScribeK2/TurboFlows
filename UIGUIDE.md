...
| Pin toggle | `.pin-button`, `.is-pinned` | `dashboard.css` | `render "workflows/pins/toggle", workflow:, pinned:, location:`, where location is `pinned`, `recent` or `play`. A plain bookmark button with a 2.5rem square target; filled primary text when pinned. Its id is `dom_id(workflow, "pin_#{location}")`, so every copy on a page is replaced after a pin change |
