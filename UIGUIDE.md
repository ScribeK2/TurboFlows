...
**pale tints only**, and appears only on badges (the step panel's type label,
the templates popover's dots) and flow-diagram nodes — never as a card border
or row background (surfaces stay neutral). **The builder's step list carries
no step colour at all** (2026-09-24): its numbers are neutral and a step's type
is a grey `.builder__step-type-icon` (from `WorkflowsHelper::STEP_TYPE_ICONS`)
on the row, in the type picker and on a jump chip. In the list, colour means a
problem. Seven solid hues down the page were noise, three pairs of them
(Action/Resolve, Message/Sub-Flow, Escalate/Form) measure under 0.05 apart in
OKLab and read as one colour at a glance, and Escalate's hue is the error red:
...
It is gone: the outline now shows a pressable `.builder__door-stub` chip per
unwired answer, which names the answer and fixes it in one click instead of
announcing a problem. Do not bring the pill back — see the Outline door chips
entry below.
...
|-----------|---------|------|-------|
| Guidance callout, reference link | `.step-guidance` (`__icon`, `__body`, `__label`, `__text`), `.step-reference`, `.step-reference-link` (`__label`) | `runner.css` | A step's `help_text` and `reference_url` in both runners (`runner/_step_body`). **Guidance** is what the agent must not miss, mostly hard rules ("Do not process the delete"), so it is the one amber thing on a runner card: `--color-warning-soft` fill, a 3px `--color-warning` left edge, the `exclamation-triangle` icon and a "Guidance" label, text in full `--color-ink`, `role="note"` labelled by that label. It was grey text on a grey box, quieter than the step's own instructions. The triangle is guidance's alone: the Escalate card uses `arrow-up-circle`, as the builder does. **Reference link:** a regular-size `.btn--secondary` naming the site it opens (`RunnerHelper#runner_link_label`: the host without `www.`, "Open link" when there is none; the full address in `title`), a link icon and an `arrow-top-right-on-square` saying it leaves the run, and "(opens in a new tab)" for screen readers. These links are tools opened mid-call (a ticket system, a lookup), which "More info" never named. Contrast of both pairs is measured in `token_contrast_test.rb` |
...
| Outline door chips | `.builder__node`, `.builder__outline-branch`, `.builder__outline-door` (`--connector`, `--jump`, `--stub`, `--extra`, `--continue`, `--fold`), `.builder__outline-connector`, `.builder__step-type-icon`, `.builder__outline-chip` (`--expression`), `.builder__door-stub`, `.builder__door-stub-text`, `.builder__outline-jump`, `.builder__outline-step-ref`, `.builder__ways-in`, `.builder__ways-in-label`, `.builder__outline-fold`, `.builder__outline-fold-count`, `.builder__outline-section`, `.builder__list-tools`, `.builder__outline-toggle-all` | `builder.css` | The step list is an outline of the graph (`workflows/_step_outline`, `_step_node`). **Tree roles:** a wrapper inside `#steps-list` (`.builder__outline`, rendered only when there are steps, so the empty state is never a tree's child) is `role="tree"`; the Unconnected section is `.builder__outline-unconnected`, a `role="group"` labelled by its heading, whose trees are one `aria-level` down; each `.builder__node` is a `role="treeitem"` with `aria-level` and `tabindex="0"` (every step is a Tab stop, and `outline-keys` gives it Enter/Space to open and the arrow keys to move and fold; its focus ring is drawn on its own row, not around the whole branch) holding the step's row, then its exits, then its continuation door (whose step is the node's next sibling); each `.builder__outline-branch` is a `role="group"` indented down a `--color-border` hairline guide, which darkens to `--color-border-strong` on every branch holding the selected row (`:has(.builder__step--selected)`), so the author can see which branch they are in at depth. `.builder__outline-door` is one door's line: `--connector` a wired continuation (see below), `--jump` wired to a step placed elsewhere, `--stub` unwired, `--extra` a connection no door claims, `--continue` the continuation (after the exits, at the parent's indent), `--fold` the door as a fold's `<summary>`. `.builder__outline-chip` is the pill: `--color-canvas-raised` fill, `--color-border` hairline, `--color-ink` text, ellipsised past 20rem with the full label in `title`. It is for choices (an exit's answer) and for stubs and jumps. **Continuation:** not a choice, so not a pill. A labelled continuation is `.builder__outline-door--connector` holding `.builder__outline-connector`, a muted "↓ Anything else" in the number column; a plain wired Next draws nothing, since its step is the very next row (an unwired one keeps its stub). **No type dots:** the list's only type marker is `.builder__step-type-icon`; a jump chip carries its target's, so the author sees what is behind a door without opening it. **Severity:** a health error or warning rings the row's number (an `outline` in `--color-negative` / `--color-warning`) and its `.step-warning-icon` follows the title; the row's left edge belongs to the selection alone. It was a coloured left border, which drew a bracket off the list's edge and sat far from the icon that explained it. **Stub:** ONE `button.builder__outline-chip.builder__door-stub` (`No → add step`), dashed `--color-border-strong` hairline and `--color-primary-text`, never a warning colour, so unfinished never reads as wired; `.builder__door-stub-text` is its view-mode reading ("No → nothing yet"), plain muted words with no pill. **Jump:** ONE `button.builder__outline-jump` naming the step it leads to (`Yes → Working · step 4`), which opens that step and selects its row. It is inline flow, not flex: a flex container blockifies its text runs and broke its reading into lines. It wraps rather than ellipsising (a two-line chip takes `--radius-lg` corners), and `.builder__outline-step-ref` keeps "· step N" one unbreakable run, so the number is never what gets cut at panel-open width. **Long unbroken runs:** the row title (`.builder__step .list-row__title`), a jump chip, `.builder__outline-fold-count` and `.builder__outline-extra-target` take `overflow-wrap: anywhere`, so a URL or ticket id in a title wraps inside its row instead of running under the warning icon and Remove. **Extra:** a dashed pill under the exits; one whole comparison reads through `format_condition_for_display` (`tier is "gold"`, raw in `title`), anything else stays raw with `--expression` (mono), so an expression never passes for an answer. **Merges share one treatment from either end:** the jump chip and `.builder__ways-in` ("2 ways in" after a row's title, sources in its `title`, part of the treeitem's name) both use the link icon in `--color-primary-text`. With a panel open, `.builder__ways-in-label` (the words) is visually hidden with the `.sr-only` recipe and the icon and count stay. **Folds:** an exit whose branch holds steps is `details.builder__outline-fold`, open by default; its `summary.builder__outline-door--fold` holds the chip plus `.builder__outline-fold-count` ("→ Working · step 4 · 1 step", shown only while closed), which wraps the same way with "· step N · K steps" in `.builder__outline-step-ref` and drops to its own line under the chip when less than 10rem is left beside it (the closed summary is `flex-wrap: wrap`, and its chip is capped short of the line so it never wraps away from its chevron); the chevron is a rotated CSS pseudo-element in `--color-ink-muted`, not an icon font, and chips with nothing to fold keep its column. `.builder__outline-section` is the "Unconnected: not reached from the start" heading. `.builder__list-tools` sits above the list and holds `.builder__outline-toggle-all` (Collapse all / Expand all, labelled by what it will do); both are hidden when the workflow has no fold, so no empty strip sits above a linear list. `outline-fold` (mounted on the builder root, not the list) keeps folds across re-renders |
...
| API token reveal | `.api-token-reveal__code` | `cards.css` | `profiles/api_tokens/_reveal`: the raw token shown once, inside a `.card`. One unbroken 50+ character run with no spaces, so it needs the same long-unbroken-run treatment as `.builder__step .list-row__title` etc. — `overflow-wrap: anywhere` — or it forces the card, and every ancestor up to `.page-content`, wider than a narrow viewport and opens horizontal scroll on the whole page |
...
| `step-list` | The type picker. A control that grows a step from a door carries `data-grow-from`, `-label`, `-condition` and `-context` (and, on an outline stub chip, `-connect-url`, which shows "An existing step…"); the picker copies them into its hidden fields when it OPENS (choosing a type closes it before the form submits, so writing on close sends a grow with no parent). "An existing step…" dispatches `step-list:pick-existing` to the list target picker | `click->step-list#growFromDoor` on an outline stub chip; a panel button outside the list element is picked up by a document listener |
| `outline-fold` | Keeps the outline's folded exit branches folded across list re-renders, reveals the branch holding the open step when that step changes or the list re-renders (a fold the author makes in between sticks), and drives Collapse all / Expand all. Records folds from `<summary>` clicks, never from `toggle` | On the builder root (a list-level mount would be discarded when a grow, a refused grow, the transitions endpoint, a delete or a health fix replaces the whole `#step-list`); `outline-fold#toggleAll` on the Collapse all button, whose target is `toggleAll` |
| `ordinal-sync` | Keeps every step number printed outside the list (a `[data-step-ordinal]` span in the panel's door rows and Use existing… candidates, the health panel's "Step N", View Flow) equal to that step's row badge after any list re-render, since any delete, grow or rewire can renumber the outline | On the builder root beside `outline-fold`; nothing to wire — mark a number with `data-step-ordinal="<step uuid>"` and it follows the list |
| `outline-keys` | The step list from the keyboard: with focus on a step's node (a treeitem, a Tab stop), Enter/Space open its panel, Down/Up move to the next/previous visible step in reading order (a closed fold's steps are skipped, chips never count), Home/End to the first/last, Left folds the branch the step sits in (through its `<summary>`, so `outline-fold` records it) and focuses that fold, Right unfolds the step's own folded branches. Keys aimed at a chip, fold or Remove are left alone | On the builder root beside `outline-fold`; nothing to wire |
| `step-target-picker` | The "Use existing…" `<dialog>` on a door row: which door it is for, the filter, and a refusal rendered inside the dialog. Mounted a second time for the list's "An existing step…" dialog (`workflows/_list_target_picker`) | On the panel body, and `step-list:pick-existing@window->step-target-picker#openFromList` on the list dialog's wrapper; `click->step-target-picker#open` on the door's button, `keydown.esc->step-target-picker#stopEscape` and `click->step-target-picker#backdropClose` on the dialog |
...
| `app/views/workflows/_step_row.html.erb` | Dense list row | Composing `.list-row` + `.list-row--compact` with block-specific concerns, rather than redefining a row. Status/meta sits inline, not on a second line — a scanned list pays for two-line rows in steps visible at once. It is one step's own line inside the outline; where it leads is the outline's job |
