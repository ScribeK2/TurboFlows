...
| Outline door chips | `.builder__node`, `.builder__outline-branch`, `.builder__outline-door` (`--step`, `--jump`, `--stub`, `--extra`, `--continue`, `--fold`), `.builder__outline-chip`, `.builder__outline-jump`, `.builder__outline-fold`, `.builder__outline-fold-count`, `.builder__door-stub`, `.builder__door-stub-text`, `.builder__ways-in`, `.builder__outline-section`, `.builder__list-tools`, `.builder__outline-toggle-all` | `builder.css` | The step list is an outline (`workflows/_step_outline`, `_step_node`): each step is a `role="treeitem"` node holding its row, its exit doors in a `role="group"` down a hairline guide, then its continuation door, whose step is the next sibling. A door is a chip: a wired one is text beside the target's type dot; a stub is ONE `button.builder__outline-chip.builder__door-stub` (`No → add step`) that opens the type picker, dashed hairline and link colour, never a warning colour; a jump is ONE `button.builder__outline-jump` naming the step it leads to (`Yes → Working · step 4`), which opens that step. The jump is inline flow, not flex: a flex container blockifies its text runs and broke its reading into lines. `__door-stub-text` is the view-mode reading of a stub. A step two or more connections lead into shows `.builder__ways-in` ("2 ways in", the sources in its `title`). Steps nothing leads to follow an `Unconnected` section heading. An exit whose branch holds steps is foldable: `details.builder__outline-fold` wraps it, `summary.builder__outline-door--fold` holds the chip plus `.builder__outline-fold-count` ("→ Working · step 4 · 1 step", hidden while open), and the chevron is a rotated CSS pseudo-element, not an icon font. `.builder__list-tools` sits above the list and holds `.builder__outline-toggle-all` (Collapse all / Expand all), hidden when the workflow has no fold; `outline-fold` (mounted on the builder root, not the list, since a delete/grow/health-fix replaces the whole list) keeps folds across re-renders and always reveals the branch holding the open panel's step |
