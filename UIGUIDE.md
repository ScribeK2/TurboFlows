...
| `outline-fold` | Keeps the outline's folded exit branches folded across list re-renders, always reveals the branch holding the open step, and drives Collapse all / Expand all. Records folds from `<summary>` clicks, never from `toggle` | On the builder root (a list-level mount would be discarded when a grow, a refused grow, the transitions endpoint, a delete or a health fix replaces the whole `#step-list`); `outline-fold#toggleAll` on the Collapse all button, whose target is `toggleAll` |
