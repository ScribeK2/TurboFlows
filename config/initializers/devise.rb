...
Devise.setup do |config|
  ...
  # random tokens. Changing this key will invalidate all existing confirmation,
  # reset password and unlock tokens.
  ...
  # :mongoid (bson_ext recommended) for MongoDB.
  # Devise automatically detects Active Record, so we don't need to set this.
  ...
  config.pepper = Rails.application.credentials.devise_pepper || Rails.application.secret_key_base
  ...
  # The time you want to timeout the user session without activity. By default
  # it will be timeout after 30 minutes. Note that this is only active if
  ...
  # When using Devise inside an engine, let's say it's `MyEngine`, and this engine
