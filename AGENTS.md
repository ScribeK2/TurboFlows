...
- The API/MCP branch ships two additive migrations (`api_tokens`, `workflows.api_token_id`) and one new runtime gem, `mcp`; `/mcp` relies on `config.hosts`, so curl it once from inside the network right after deploying; nothing under `/api` or `/mcp` is reachable until someone creates a token; tell IT and the AI team what a `tf_live_` token is before announcing it
