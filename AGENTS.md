...
nothing at all is wired, since `:no_outgoing_transitions` already says that
once, and empty again once a fallback is catching them. `#unmatched_extras` is an extra whose condition names a value the
...
browser, generated by the model for every other writer, `attr_readonly`,
unique): `known` is every uuid the editor has ever shown, `rows` is what it
