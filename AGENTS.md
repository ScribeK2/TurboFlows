...
- The builder outline (2026-09) has no migration and no cutover hazard: a tab open across the deploy shows the old flat list until it reloads, and its writes are the same writes. Two small edges, neither losing data: the first list broadcast it receives renders the outline under the OLD JavaScript (folds do not survive a re-render, and there is no "An existing step…"), and a drag-reorder made before that hits the deleted `reorder` route and changes nothing
