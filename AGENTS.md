...
## What is TurboFlows?
A straightforward workflow creator for call/chat centers to build, simulate, and manage post-onboarding training + client troubleshooting flows in a builder whose step list is an outline of the graph.
...
- The builder's step list is an outline of that graph: each step's last door continues at the parent's indent and the others nest; numbers follow the outline's reading order, derived on every read. In full: `docs/agents/builder.md` § The list is an outline
...
| `/api/v1`, `/mcp`, API tokens (`ApiToken`, profile and admin token pages), `Api::DraftBodyGuard`, the API's `Rack::Attack` rules, OpenAPI, `/api/docs` | `docs/agents/api.md` |
...
- The builder outline (2026-09) has no migration and no cutover hazard, but a tab open across the deploy does not simply keep the old flat list: its first list stream or broadcast (its own grow, or a collaborator's structural save) swaps in outline markup under the tab's OLD JavaScript and stylesheet. Harmless: the chips render unstyled, the old `step-list#growFromDoor` and `builder#openStep` still answer them, "An existing step…" is absent and folds do not persist. A drag-reorder from that tab hits the deleted `reorder` route and writes nothing (the old JS ignores the 404). A reload brings the outline proper
- The API/MCP branch ships two additive migrations (`api_tokens`, `workflows.api_token_id`) and one new runtime gem, `mcp`; `/mcp` relies on `config.hosts`, so curl it once from inside the network right after deploying; no workflow data is reachable, and nothing can be written, until someone creates a token (the OpenAPI document and `/api/docs` are token-free by design); tell IT and the AI team what a `tf_live_` token is before announcing it
